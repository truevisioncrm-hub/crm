...
export default function AgentProfilePage() {
    ...
    useEffect(() => {
        console.log("AgentProfilePage Mounted, User:", user);
    }, [user]);
        ...
        <div className="max-w-2xl space-y-8 animate-fade-in mx-auto">
            ...
            <div className="text-center">
                <div className="relative inline-block">
                    <div className="w-24 h-24 rounded-full bg-neutral-100 flex items-center justify-center text-3xl font-bold text-neutral-400 border-4 border-white shadow-xl mb-4 overflow-hidden">
                        {user?.avatar ? <Image src={user.avatar} alt="Avatar" width={96} height={96} className="w-full h-full object-cover" /> : (user?.name?.charAt(0) || "U")}
                    </div>
                ...
                <h1 className="text-2xl font-bold text-neutral-900">{user?.name || "Agent"}</h1>
                <p className="text-sm text-neutral-500">{user?.role === 'agent' ? 'Real Estate Agent' : 'Admin'}</p>
            </div>
            ...
            <div className="bg-white border border-neutral-200 rounded-3xl p-8 shadow-sm card-shadow space-y-6">
                <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                        ...
                        <input type="text" defaultValue={user?.name || ""} className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-medium" />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Area/Location</label>
                        <input type="text" defaultValue={user?.area || "N/A"} className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-medium" />
                    </div>
                    <div className="col-span-2 space-y-1.5">
                        <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Email Address</label>
                        <input type="email" defaultValue={user?.email || ""} className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all font-medium" />
                    </div>
                </div>
